...
```
  ┌────────────┐   TCP :1337    ┌─────────────────────┐   TCP :5432   ┌────────────┐
  │  App /     │ ─────────────▶ │      PgSiphon       │ ────────────▶ │  Postgres  │
  │  ORM /     │ ◀───────────── │  (transparent proxy)│ ◀──────────── │   server   │
  │  psql      │                └──────────┬──────────┘               └────────────┘
  └────────────┘                           │ (take copy of client -> server bytes)
                                           ▼
                                  (sidecar) decode bytes -> pub/sub -> UI (or CSV recording)
```
...
<<50 00 00 00 1C 00 53 45 4C 45 43 54 20 2A 20 46 52 4F 4D 20 6E 61 6D 65 73 3B 00 00 00 44 00 00 00 06 53 00 48 00 00 00 04 >>
```
